...
# Version: 0.9.1 
# Author: fate by god
# Created Time : 2021-01-14 16:53:09
...
cat_tomcat_log() {
    ...
    if [[ ! "$1" =~ ^-l ]] ; then
        if [ "$2" != "" ] ; then
            tomcat_log=`find /opt/ -type d -maxdepth 2 2>/dev/null |grep tomcat|grep $2|egrep -v "$B_sdvgclude_values"`
    ...
    mul_values_n=(`for i in ${mul_tomcat_log[*]} ; do
        local n=$(echo "$T_mul_cS $T_mul_label $T_mul_t $i/logs/catalina.out")      
        echo $n ; done`)
