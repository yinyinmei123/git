...
B_exclude_process="catalina.sh|cronolog|grep|tail"
...
# Version: 0.9.4.1
# Author: fate by god
# Created Time : 2021-02-18 15:28:09
...
A_VERSION="kiserver_version=0.941"
H_VERSION="0.9.4.1"
    ...
    for ((i=0;i<${#tomcat_pid[*]};i++)) ; do
        if [ ! "${tomcat_pid[i]}" = "NO_PID" ] ; then
            if [ -e ${tomcat_path[i]}/bin/catalina.sh ] ; then
                ...
                [ $yy = 2 ] && echo " >>> ${server_name[i]} 1 done" || echo " >>> ${server_name[i]} 1 fail"
                xx=`sed -n '/cronolog/p' ${tomcat_path[i]}/bin/catalina.sh |wc -l`
                [ $xx = 2 ] && echo " >>> ${server_name[i]} 2 done" || echo " >>> ${server_name[i]} 2 fail"
            else
                echo "cannot found ${tomcat_path[i]} catalina.sh"
            fi
        else
            echo " >>> ${server_name[i]} is no runing "
        fi    
    done
