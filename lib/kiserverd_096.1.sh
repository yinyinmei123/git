...
sleep 1
echo " [ Kiserverd ] INFO >>> Checking dependencies ncurses-devel "
if [ `rpm -qa ncurses-devel|wc -l` = 0 ] ; then
    echo " [ Kiserverd ] INFO >>> Installing dependencies ncurses-devel "
    yum install -y ncurses-devel >/dev/null 2>&1
    if [ $? -ne 0 ] ; then
        echo " [ Kiserverd ] ERROR >>> Dependency ncurses-devel installation failed " 
        v_ncurses_devel=1
    else
        echo " [ Kiserverd ] INFO >>> Dependency ncurses-devel installed successfully " 
    fi
fi
echo " [ Kiserverd ] INFO >>> Checking dependencies multitail "
if [ ! -e /usr/bin/multitail ] ; then
    echo " [ Kiserverd ] INFO >>> Installing dependencies multitail "
    yum install -y multitail >/dev/null 2>&1
    if [ $? -ne 0 ] ; then
        echo " [ Kiserverd ] ERROR >>> Dependency multitail installation failed " 
        v_multitail=1
    else
        echo " [ Kiserverd ] INFO >>> Dependency multitail installed successfully " 
    fi
fi
echo " [ Kiserverd ] INFO >>> Checking dependencies cronolog "
if [ `rpm -qa cronolog|wc -l` = 0 ] ; then
    echo " [ Kiserverd ] INFO >>> Installing cronolog dependencies cronolog "
    yum install -y cronolog
    if [ $? -ne 0 ] ; then
        echo " [ Kiserverd ] ERROR >>> Dependency installation failed " 
        v_cronolog=1
    else
        echo " [ Kiserverd ] INFO >>> Dependency installed successfully " 
    fi
fi
...
B_exclude_values="bin|webapps|conf|work|logs|lib|temp|File|backup"

B_exclude_process="catalina.sh|cronolog|grep|tail|process|-m|--log|vim"

B_check_values="tomcat"

B_include_httpprot=protocol\=\"HTTP\/1.1\"

B_process_kill="monitor|vim|grep|process"

B_process_kill_all="vim|grep|process"
...
FRONT AND APPLICATION
     ...
     --conf
           配置kiserverd的基本参数，用来调试与应用。

     -cutlog
           此参数用来切割tomcat服务日志，主要内容是改变了tomcat中catalina.sh的生成日志方式。
           需注意，目前仅测试为tomcat8.0以上的版本可用
           在切割完成以后需要重启各个tomcat服务。
     ...
     --log 
          用来查看kiserverd的运行日志。可以查看各式各样的输出。
 
     -lsz
          在切割完成后的日志，一键快捷下载当日的tomcat日志。

     -i/-immediately
         用来直接解压文件，且直接重启服务。
         相对而言此参数不用启动后台的监测服务解压重启，而是使用了单次的启动方式，用来临时使用。
     ...
     --ssh_passwd_done
            用来一键化配置服务器主机之间的相互免密通信。
            需要配置文件，配置文件的路径在/tomcat_file/conf/下
            配置文件的写法需要参考说明。
 
    --ssh_check
            用来检测免密是否成功。在参数后面更相应的命令。

    --ssh_installs
            在免密成功之后，用来一键化免密安装此工具到其他服务器上。
...
cat > $K_BIN/kiserverd_port.sh <<'EOF'
[ -f ${n2}/conf/server.xml ] && {
egrep -n "<Connector" ${n2}/conf/server.xml|awk -F ":" '{print $1}' > $K_WORKTMP/server.tmp
n=`cat $K_WORKTMP/server.tmp |wc -l`
http_rowid_mun=(`cat $K_WORKTMP/server.tmp`)
for ((s=0;s<$n;s++)) ; do
    a=`sed -n "$((${http_rowid_mun[s]}-1))p" ${n2}/conf/server.xml |grep  '<!--'|wc -l`
    sed -n "${http_rowid_mun[s]},$((${http_rowid_mun[s]}+4))p" ${n2}/conf/server.xml >$K_WORKTMP/server2.tmp
    if [ $a = 1 ] ; then
        b=`sed -n "$((${http_rowid_mun[s]}-1))p" ${n2}/conf/server.xml |grep  '<!--'|grep '>'|wc -l`
        if [ $b = 0 ] ; then
            continue
        fi
        c=`cat $K_WORKTMP/server2.tmp|egrep 'protocol="HTTP/1.1'|wc -l`
        if [ $c = 0 ] ; then
            continue
        fi
    fi
    d=`cat $K_WORKTMP/server2.tmp |sed -n "/port=/s/^.*port=\"\([^ \"]*\).*/\1/p"`
done }
EOF

cat > $K_CONF/livenessProde.ini.bak <<'EOF'
### 这个是一个模板的探针服务配置文件。
### 如果你希望启用该服务，则需要把自己编写一个配置文件。
### 现在提供三种探针的监测服务。httpget类型是监测一个页面是否正常。
###                             tcpsocket类型是监测服务的端口是否正常。
###                             exec类型是使用命令的方式监测是否正常，监测结果将根据你输入的命令的返回值来进行判断。


[server1]
servername = tomcat-app-2
label = tomcat-app-2
...
[server3]
servername = apache-tomcat-8.5.60
label = apache-tomcat-8.5.60
livenessProbe = exec
command = "cat /root/123.qq"
checkperiod = 5
checkrepeat = 5
restartedwait = 60
...
successful_fail() {
    [ $retval -eq 0 ] && {
    daemon_process &>/dev/null
    new_tomcat=`ps -ef|grep "${startGJZ}/"|egrep -v "$B_exclude_process"|awk '{print $2}'`
...
# Version: 0.9.6.1
# Author: fate by god
# Created Time : 2021-04-23
# Description: 1.调整部分功能
EOF

A_VERSION="kiserver_version=0.961"
H_VERSION="0.9.6.1"
...
status() {
    ...
    # echo "╠══╗║║╚║║═╠╮║║╠═ ╠╣  ║╰═╮║╰╯║╰╮╭╯║╰═╮"
    # echo "╯╰═╝╰╩╰╚╯ ╚╝╚╚╚═══╯  ╰══╯╰══╯ ╰╯ ╰══╯"
    echo -e "\033[036m    < KISERVER TOMCAT STATE >\033[0m                            $message"
case "$1" in
    -status)
...
kiserverd_process() {
    ...
    local clear=true
    kiserverd_process_pid=`ps -ef|grep kiserverd|egrep -v "grep|process|tail|log|-m|--log"|awk '{print $2}'`
    kiserverd_process_pid_num=`ps -ef|grep kiserverd|egrep -v "grep|process|tail|log|-m|--log"|awk '{print $2}'|wc -l`
    if [ "$kiserverd_process_pid" != "" ]
    then
        kiserverd_process_keyword=`ps -ef|grep kiserverd|egrep -v "grep|process|tail|log|-m|--log"|awk '{print $12}'|sed -n "${kiserverd_process_num}p"`
        max=0
        ...
        do
            kiserverd_process_pid=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $2}'|sed -n "${kiserverd_process_num}p"`
            kiserverd_process_keyword=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $12}'|sed -n "${kiserverd_process_num}p"`
            kiserverd_process_server=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $11}'|sed -n "${kiserverd_process_num}p"`
            kiserverd_process_mode=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $13}'|sed -n "${kiserverd_process_num}p"`
            kiserverd_process_control=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $14}'|sed -n "${kiserverd_process_num}p"`
            kiserverd_process_control_time1=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $15}'|sed -n "${kiserverd_process_num}p"`
            kiserverd_process_control_time2=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $16}'|sed -n "${kiserverd_process_num}p"`
			kiserverd_process_control_unzip=`ps -ef|grep kiserverd|egrep -v "$B_exclude_process"|awk '{print $17}'|sed -n "${kiserverd_process_num}p"`
            ...
            echo "| SERVER NAME $server_name_title_n|  STATE  |  PID   | ST      | CONTROL | TIME  | UNZIP |     CONTROL DIR $control_dir_title_n|" && \
            echo +$rung_server_name+---------+--------+---------+---------+-------+-------+$rung_control_dir+
            ...
            if [ "$kiserverd_process_control" = "fixed" ];then
                if [ "$kiserverd_process_control_time1" -le `date +%H` ];then
                    ...
                    printf_B_end="\e[0m"
				elif [ "$kiserverd_process_control_time1" -eq `date +%H` ];then
			...
			if [ "$kiserverd_process_control_unzip" = "1" ];then
			    kiserverd_process_control_unzip=YES
			elif [  "$kiserverd_process_control_unzip" = "" ];then
			    kiserverd_process_control_unzip=YES
			else
			    kiserverd_process_control_unzip=NO
			fi
			
            printf "%s %-$((${max}+4))s %s $printf_blue%s$printf_B_end %s %-6s %s %-7s %s %-7s %s $printf_B_yellow%s$printf_B_end %s %-5s %s %-$((${max}+26))s %s %s\n"                   \
				   ...
				   "|" "$kiserverd_process_control_unzip"                                    \
...
kiserverd_process_kill() {
    kiserverd_process_pid=`ps -ef|grep kiserverd|egrep -v "$B_process_kill"|awk '{print $2}'`
    if [ "$2" = "--all" ] ; then
        kiserverd_process_pid=`ps -ef|grep kiserverd|egrep -v "$B_process_kill_all"|awk '{print $2}'`
    fi
    if [ "$kiserverd_process_pid" != "" ]
    then
...
cat_tomcat_log() {
    ...
    if [ ! "$mul_include" = "null" ] ; then
        T_mul_e="-e \"$mul_include\""
    fi
    if [ ! "$mul_exclude" = "null" ] ; then
        T_mul_ev="-ev \"$mul_exclude\""
    fi
    
    # if [ "$1" = "^-[0-9]*l$" ] ; then
    if [[ ! "$1" =~ ^-l ]] ; then
        if [ "$2" != "" ] ; then
            tomcat_log=`find $server_path -type d -maxdepth 2 2>/dev/null |grep tomcat|grep $2|egrep -v "$B_exclude_values"`
            eval tail -${tail_num}f ${tomcat_log}/logs/catalina.out.`date +%Y-%m-%d`
            exit 0
        else
            kiserverd_process_keyword=`ps -ef|grep kiserverd|egrep -v "grep|process"|awk '{print $12}'`
            tomcat_log=`find $server_path -type d -maxdepth 2 2>/dev/null |grep tomcat|grep $kiserverd_process_keyword|egrep -v "$B_exclude_values"`
            eval tail -${tail_num}f ${tomcat_log}/logs/catalina.out.`date +%Y-%m-%d`
        exit 0
        fi
    fi
    ...
    elif [ ! "$mul_buffer_size" = "null" ] ; then
    ...
    mul_tomcat_log=(`find $server_path -type d -maxdepth 2 2>/dev/null |grep tomcat|egrep "$process_keyword"|egrep -v "$B_exclude_values"`)
    mul_values_n=(`for i in ${mul_tomcat_log[*]} ; do
        local n=$(eval echo "$T_mul_cS $T_mul_m $T_mul_mb $T_mul_label $T_mul_t $i/logs/catalina.out.$today_date")      
        echo $n ; done`)
    echo "multitail $T_mul_F $T_mul_s $T_mul_sn $T_mul_ev $T_mul_e ${mul_values_n[*]} $T_mul_R $T_mul_l"
    eval multitail $T_mul_F $T_mul_s $T_mul_sn "$T_mul_ev" "$T_mul_e" ${mul_values_n[*]} $T_mul_R "$T_mul_l"
    exit 0
...
download_log() {
    [ -z $2 ] && echo "Usage : you must input keyword-server-name " && exit 1 
    today_log=`date +%Y-%m-%d`
    sz_path=`find $server_path -type d -maxdepth 2 2>/dev/null |grep tomcat|grep $2|egrep -v "$B_exclude_values"`
    sz $sz_path/logs/catalina.out.$today_log
}
...
# 定时切割任务，暂需手动重启tomcat服务
cutlog() {
    [ ! "$C_cut_tomcatlog" = 1 ] && echo " >>> conf fail" && exit 1
    select_value
    for ((i=0;i<${#tomcat_pid[*]};i++)) ; do
        if [ ! "${tomcat_pid[i]}" = "NO_PID" ] ; then
            if [ -e ${tomcat_path[i]}/bin/catalina.sh ] ; then
                sed -i '/org.apache.catalina.startup.Bootstrap "$@" start \\/c org.apache.catalina.startup.Bootstrap "$@" start 2>&1 \\' ${tomcat_path[i]}/bin/catalina.sh && \
                sed -i '/>> "$CATALINA_OUT" 2>&1 "&"/c \| cronolog "${CATALINA_OUT}.%Y-%m-%d" >> \/dev\/null &' ${tomcat_path[i]}/bin/catalina.sh
                yy=`sed -n '/org.apache.catalina.startup.Bootstrap "$@" start 2>&1/p' ${tomcat_path[i]}/bin/catalina.sh |wc -l`
                [ $yy = 2 ] && echo -e " >>> \033[032mINFO\033[0m  [ ${server_name[i]} ] 50%  done" || echo " >>> \033[031mERROR\033[0m [ ${server_name[i]} ] 1 fail"
                xx=`sed -n '/cronolog/p' ${tomcat_path[i]}/bin/catalina.sh |wc -l`
                [ $xx = 2 ] && echo -e " >>> \033[032mINFO\033[0m  [ ${server_name[i]} ] 100% done" || echo " >>> \033[031mERROR\033[0m [ ${server_name[i]} ] 2 fail"
            else
                echo -e " >>> \033[031mERROR\033[0m cannot found ${tomcat_path[i]} catalina.sh"
            fi
        else
            echo -e " >>> \033[033mINFO\033[0m  [ ${server_name[i]} ] is no runing "
        fi    
    done
...
tomcat_pid=(`for n in ${tomcat_path[*]} ; do
    local nn=$(ps -ef|grep "${n}/"|egrep -v "$B_exclude_process"|awk '{print $2}')
    [ "$nn" = "" ] && nn="NO_PID"
    echo $nn
done`)
...
while true ; do
    ...
    server_cpu=(`for n in ${tomcat_path[*]} ; do
        local nn=$(ps aux|grep ${n}/|egrep -v "$B_exclude_process" |awk '{print $3}')
        [ "$nn" = "" ] && {
        local nn="NO_PID" ; }
        echo $nn 
        done`)
...
#### 探针监测
liveness_httpget() {
    H_tomcat_select=`cat $K_WORKTMP/tomcat_full_path.dir|eval grep "$"liveness_servername_${i}`
    while true ; do
        while [ `eval curl -I -m 10 -o /dev/null -s -w %{http_code} 127.0.0.1:"$"liveness_port_${i}"$"liveness_path_${i}` -gt 399 ] ; do
            H_http_code=`eval curl -I -m 10 -o /dev/null -s -w %{http_code} 127.0.0.1:"$"liveness_port_${i}"$"liveness_path_${i}`
            local h_h=0
            for ((n1=0;n1<20191226;n1++)) ; do
                daemon_process &>/dev/null
                H_http_code=`eval curl -I -m 10 -o /dev/null -s -w %{http_code} 127.0.0.1:"$"liveness_port_${i}"$"liveness_path_${i}`
                if [ $H_http_code -gt 399 ] ; then
                    ((z++))
                    eval echo -n [ $ltime - "$"liveness_servername_${i} ] >>$K_LOG/kiserverd.log ; echo -e " $B_yellow`eval echo 检索到服务"$"liveness_servername_${i}异常，正在重新检测，当前重试次数$((h_h))`$B_end" >>$K_LOG/kiserverd.log
                    if [ $z -gt 2 ] ; then
                        sleep 5
                        eval echo -n [ $ltime - "$"liveness_servername_${i} ] >>$K_LOG/kiserverd.log ; echo -e " $B_red`eval echo 检索到服务"$"liveness_servername_${i}异常，即将重新启动该服务`$B_end" >>$K_LOG/kiserverd.log
                        eval /usr/bin/kiserverd -p tomcat "$"liveness_servername_${i} restart  >>$K_LOG/kiserverd.log
                        eval sleep "$"liveness_restartedwait_${i}
                        h_h=0
                    fi
                fi
                # echo $h_h
                eval sleep "$"liveness_checkperiod_${i}
            done
        done
            
        while [ `eval curl -I -m 10 -o /dev/null -s -w %{http_code} 127.0.0.1:"$"liveness_port_${i}"$"liveness_path_${i}` -eq 000 ] ; do
           H_http_code=`eval curl -I -m 10 -o /dev/null -s -w %{http_code} 127.0.0.1:"$"liveness_port_${i}"$"liveness_path_${i}`
           echo 
           if eval [ "$"liveness_livenessProbe_$i = httpget ] ; then
               eval echo 无法连接"$"liveness_servername_${i}的端口"$"liveness_port_${i},请确认是否启动服务。
               exit 1
           fi
           local h_t=0
           if eval [ "$"liveness_livenessProbe_$i = tcpsocket ] ; then
               for ((n2=0;n2<20191226;n++)) ; do
                   eval curl -I -m 10 -o /dev/null -s -w %{http_code} 127.0.0.1:"$"liveness_port_${i}
                   if [ $? -ne 0 ] ; then
                       ((h_t++))
                       eval echo -n [ $ltime - "$"liveness_servername_${i} ] >>$K_LOG/kiserverd.log ; echo -e " $B_yellow`eval echo 检索到服务"$"liveness_servername_${i}异常，正在重新检测，当前重试次数$((h_t))`$B_end" >>$K_LOG/kiserverd.log
                       eval sleep "$"liveness_checkperiod_${i}
                       if eval [ $h_t -gt "$"liveness_checkrepeat_${i} ] ; then
                           eval echo -n [ $ltime - "$"liveness_servername_${i} ] >>$K_LOG/kiserverd.log ; echo -e " $B_red`eval echo 检索到服务"$"liveness_servername_${i}异常，即将重新启动该服务`$B_end" >>$K_LOG/kiserverd.log
                           eval /usr/bin/kiserverd -p tomcat "$"liveness_servername_${i} restart  >>$K_LOG/kiserverd.log
                           local h_t=0
                           eval sleep "$"liveness_restartedwait_${i}
    ...
    #if [ $H_http_code -gt 399 ] ; then
    #    /usr/bin/kiserverd -p tomcat $liveness_servername_1 restart 
    #fi
...
liveness_exec() {
    local z=0 
    for ((n=0;n<20191226;n++)) ; do
        daemon_process &>/dev/null
        eval h_comcamd="$"liveness_command_$i
        $h_comcamd &>/dev/null
        if [ $? -ne 0 ] ; then
            ((z++))
            eval echo -n [ $ltime - "$"liveness_servername_${i} ] >>$K_LOG/kiserverd.log ; echo -e " $B_yellow`eval echo 检索到服务"$"liveness_servername_${i}异常，正在重新检测，当前重试次数$z `$B_end" >>$K_LOG/kiserverd.log
            while eval [ $z -gt "$"liveness_checkrepeat_$i ] ; do
                daemon_process &>/dev/null
                eval echo -n [ $ltime - "$"liveness_servername_${i} ] >>$K_LOG/kiserverd.log ; echo -e " $B_red`eval echo 检索到服务"$"liveness_servername_${i}异常，即将重新启动该服务`$B_end" >>$K_LOG/kiserverd.log
                eval /usr/bin/kiserverd -p tomcat "$"liveness_servername_${i} restart  >>$K_LOG/kiserverd.log
                eval sleep "$"liveness_restartedwait_${i}
...
health() {
    # echo "`awk -F '=' '/\[server1\]/{a=1}a==1&&$1~/servername/{print $2;exit}' /root/livenessProde.ini`"
    liveness_loop=`sed -n "/\[server.*/p" $K_CONF/livenessProde.ini | wc -l`
    # echo $liveness_loop
    for liveness_var in servername label livenessProbe path port checkperiod checkrepeat restartedwait command  ; do
        for ((i=1;i<=$liveness_loop;i++)) ; do
            a=`awk -F '= ' '/\[server'"$i"'\]/{a=1}a==1&&$1~/'"$liveness_var"'/{print $2;exit}' $K_CONF/livenessProde.ini `
            eval liveness_${liveness_var}_${i}=${a}
    #        eval echo "$"liveness_${liveness_var}_${i}
        done
    done
    ...
    for ((i=1;i<=$liveness_loop;i++)) ; do
        if eval [ "$"liveness_livenessProbe_$i = httpget ] ; then
            liveness_httpget &
        elif eval [ "$"liveness_livenessProbe_$i = tcpsocket ] ; then
            liveness_httpget &
            # echo 2
        elif eval [ "$"liveness_livenessProbe_$i = exec ] ; then
            liveness_exec &
        else
            echo 错误，请指定正确探针类型 
            exit 1
...
tomcat_server_monitor_ex() {
        ...
        do
            if [ "$control_status" = "now" ];then
                ...
                nohup /usr/bin/kiserverd -pd tomcat $n restart $control_status $control_time_h $control_time_m $unzip_01 &
            fi
            sleep 0.5
        done
        >$K_TMP/tomcat_server_name.dir.ex
...
normal_monitor_null() {
	...
	echo -e "----------------------------------------"
    if [ "$control_status" = "fixed" -o "$control_status" = "delay" ]; then
	    echo -e "\033[036m- 1 -\033[0m YES"
		echo -e "\033[036m- 0 -\033[0m NO"
	    read -t 30 -p "Do you want to unzip :  " unzip_01
		if [ $unzip_01 -nt 0 -o $unzip_01 -nt 1 ];then
		    echo "请输入正确的参数"
			exit 1
		fi
	fi
...
#!/bin/bash
    find $server_path -type d -maxdepth 2 2>/dev/null |egrep "$B_check_values"|egrep -v "$B_exclude_values" >$K_WORKTMP/tomcat_full_path.dir
...
function _cmd_kiserverd-monitor() {
    local cur="${COMP_WORDS[COMP_CWORD]}"
    local prev="${COMP_WORDS[COMP_CWORD-1]}"
    local options="${TOMCAT_SERVER_NAME[*]} -c 
	                                        --control 
											-health"
    COMPREPLY=( $(compgen -W "${options}" -- ${cur}) )
    return 0
...
v_warning='$v_ncurses_devel|$v_multitail|$v_cronolog'
if [ "$v_ncurses_devel" = 1 -o "$v_multitail" = 1 -o "$v_cronolog" = 1 ] ; then
    echo " [ Kiserverd ] Warning >>> Dependency is not installed correctly, some functions will be limited "
fi
